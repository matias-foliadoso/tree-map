...
for (const datasetName in data) {
    options.push(`<a href="?data=${datasetName}">${data[datasetName].option}</a>`)
}
document.getElementById('options').innerHTML = options.join('|')

// Url param.
const urlParams = new URLSearchParams(window.location.search)
const defaultDataset = Object.keys(data)[0]
const dataset = data[urlParams.get('data') || defaultDataset]
...
// Treemap svg.
const treeMapSvg = d3.select('.tree-map')
...
const colors = colorsHex.map(c => d3.interpolateRgb(c, '#fff')(0.2))
...
const legendElems = legend
    ...
    .enter()
    .append('g')
    .attr('transform', (d, i) => `translate(${(i % legendElemsPerRow) * LEGEND_H_SPACING}, ${
